use crate::*;

#[test]
fn heif() {
    // Complete image from libheif
    const ENCODED: &[u8] = &[
        ,
    ];

    let buf = &mut std::io::Cursor::new(ENCODED);
    let ftyp = Ftyp::decode(buf).expect("failed to decode ftyp");

    assert_eq!(
        ftyp,
        Ftyp {
            major_brand: b"heic".into(),
            minor_version: 0,
            compatible_brands: vec![b"mif1".into(), b"heic".into(), b"miaf".into()],
        }
    );

    let meta = Meta::decode(buf).expect("failed to decode meta");
    assert_eq!(
        meta,
        Meta {
            hdlr: Hdlr {
                handler: b"pict".into(),
                name: "".into()
            },
            pitm: Some(Pitm { item_id: 1 }),
            iref: Some(Iref {
                references: vec![Reference {
                    reference_type: FourCC::new(b"auxl"),
                    from_item_id: 2,
                    to_item_ids: vec![1]
                }]
            }),
            ilst: None,
            unknown: vec![
                Any::Unknown(
                    b"iloc".into(),
                    vec![
                        0, 0, 0, 0, 68, 64, 0, 2, 0, 1, 0, 0, 0, 0, 2, 144, 0, 1, 0, 0, 0, 0, 0, 0,
                        0, 43, 0, 2, 0, 0, 0, 0, 2, 187, 0, 1, 0, 0, 0, 0, 0, 0, 0, 35
                    ]
                ),
                Any::Unknown(
                    b"iinf".into(),
                    vec![
                        0, 0, 0, 0, 0, 2, 0, 0, 0, 21, 105, 110, 102, 101, 2, 0, 0, 0, 0, 1, 0, 0,
                        104, 118, 99, 49, 0, 0, 0, 0, 21, 105, 110, 102, 101, 2, 0, 0, 0, 0, 2, 0,
                        0, 104, 118, 99, 49, 0
                    ]
                ),
                Any::Unknown(
                    b"iprp".into(),
                    vec![
                        0, 0, 1, 131, 105, 112, 99, 111, 0, 0, 0, 118, 104, 118, 99, 67, 1, 3, 112,
                        0, 0, 0, 0, 0, 0, 0, 0, 0, 30, 240, 0, 252, 253, 248, 248, 0, 0, 15, 3, 32,
                        0, 1, 0, 24, 64, 1, 12, 1, 255, 255, 3, 112, 0, 0, 3, 0, 144, 0, 0, 3, 0,
                        0, 3, 0, 30, 186, 2, 64, 33, 0, 1, 0, 42, 66, 1, 1, 3, 112, 0, 0, 3, 0,
                        144, 0, 0, 3, 0, 0, 3, 0, 30, 160, 32, 129, 5, 150, 234, 174, 154, 230,
                        224, 33, 160, 192, 128, 0, 0, 3, 0, 128, 0, 0, 3, 0, 132, 34, 0, 1, 0, 6,
                        68, 1, 193, 115, 193, 137, 0, 0, 0, 19, 99, 111, 108, 114, 110, 99, 108,
                        120, 0, 1, 0, 13, 0, 6, 128, 0, 0, 0, 20, 105, 115, 112, 101, 0, 0, 0, 0,
                        0, 0, 0, 64, 0, 0, 0, 64, 0, 0, 0, 40, 99, 108, 97, 112, 0, 0, 0, 8, 0, 0,
                        0, 1, 0, 0, 0, 1, 0, 0, 0, 1, 255, 255, 255, 200, 0, 0, 0, 2, 255, 255,
                        255, 193, 0, 0, 0, 2, 0, 0, 0, 16, 112, 105, 120, 105, 0, 0, 0, 0, 3, 8, 8,
                        8, 0, 0, 0, 113, 104, 118, 99, 67, 1, 4, 8, 0, 0, 0, 0, 0, 0, 0, 0, 0, 30,
                        240, 0, 252, 252, 248, 248, 0, 0, 15, 3, 32, 0, 1, 0, 23, 64, 1, 12, 1,
                        255, 255, 4, 8, 0, 0, 3, 0, 159, 248, 0, 0, 3, 0, 0, 30, 186, 2, 64, 33, 0,
                        1, 0, 38, 66, 1, 1, 4, 8, 0, 0, 3, 0, 159, 248, 0, 0, 3, 0, 0, 30, 192,
                        130, 4, 22, 91, 170, 186, 107, 155, 2, 0, 0, 3, 0, 2, 0, 0, 3, 0, 2, 16,
                        34, 0, 1, 0, 6, 68, 1, 193, 115, 193, 137, 0, 0, 0, 14, 112, 105, 120, 105,
                        0, 0, 0, 0, 1, 8, 0, 0, 0, 39, 97, 117, 120, 67, 0, 0, 0, 0, 117, 114, 110,
                        58, 109, 112, 101, 103, 58, 104, 101, 118, 99, 58, 50, 48, 49, 53, 58, 97,
                        117, 120, 105, 100, 58, 49, 0, 0, 0, 0, 32, 105, 112, 109, 97, 0, 0, 0, 0,
                        0, 0, 0, 2, 0, 1, 5, 129, 2, 3, 5, 132, 0, 2, 5, 134, 3, 7, 136, 132
                    ]
                ),
            ]
        }
    );

    let mdat = Mdat::decode(buf).expect("failed to decode mdat");

    let mut buf = Vec::new();
    ftyp.encode(&mut buf).expect("failed to encode ftyp");
    meta.encode(&mut buf).expect("failed to encode meta");
    mdat.encode(&mut buf).expect("failed to encode mdat");

    assert_eq!(buf, ENCODED);
}
